...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Masternodecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Masternodecoin
    // Mac: ~/Library/Application Support/Masternodecoin
    // Unix: ~/.Masternodecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Masternodecoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Masternodecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Masternodecoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "masternodecoind.pid"));
