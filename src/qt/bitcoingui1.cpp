...
{
    resize(1024, 520);
    setWindowTitle(tr("Masternodecoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#if defined(MTNC_MOD)
    okbWidget = new OKBWidget(overviewPage,sendCoinsPage);
    exAppsWidget = new exAppsForm();
    pChatWidget = new pchatForm();
    pSearchWidget = new pSearchForm();
    okBountyWidget = new okBountyForm(okbWidget);
#endif
...
void BitcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(tr("&About Masternodecoin"), this);
    aboutAction->setToolTip(tr("Show information about Masternodecoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Masternodecoin"));
...
//todo 此处代码将toolsbar添加到Widget上
//todo 通用任务栏  增加按钮 apps 和 超链接组
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Masternodecoin client") + QString(" ") + tr("[testnet]"));
        ...
        // add richards okb支付
        connect(okbWidget, SIGNAL(message(QString,QString,bool,unsigned int)), this, SLOT(message(QString,QString,bool,unsigned int)));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Masternodecoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Masternodecoin network", "", count));
}
//mark richards  判断是否同步完成
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    if(secs < 90*60)
    {
        ...
        //add richards
        qobject_cast<OKBWidget *>(okbWidget)->setOutOfSync(false);//已经同步
    ...
    else
    {
        ...
        //add richards
        qobject_cast<OKBWidget *>(okbWidget)->setOutOfSync(true);//等待同步
...
{
    QString strTitle = tr("Masternodecoin") + " - ";
...
#if defined(MTNC_MOD)
void BitcoinGUI::gotoExAppsPage()
{
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Masternodecoin address or malformed URI parameters."));
    }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    if(fWalletUnlockStakingOnly)
    {
        ...
        //add richards
        qobject_cast<OKBWidget *>(okbWidget)->setWalletSta(false); //钱包未加密 可用
    }
    else
    {
        ...
        case WalletModel::Unencrypted:
            ...
            //add richards
            qobject_cast<OKBWidget *>(okbWidget)->setWalletSta(true); //钱包解密 可用
            break;
        case WalletModel::Unlocked:
            ...
            //add richards
            qobject_cast<OKBWidget *>(okbWidget)->setWalletSta(true); //钱包未加密 可用
            break;
        case WalletModel::Locked:
            ...
            //add richards
            qobject_cast<OKBWidget *>(okbWidget)->setWalletSta(false); //钱包锁定 不可用
            break;
        }
